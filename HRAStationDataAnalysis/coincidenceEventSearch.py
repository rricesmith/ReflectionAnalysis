...
def add_parameter_to_events_file_by_file(events_dict, parameter_name, date, cuts=True, flag='base'):
    ...
    a potentially huge index map in memory. (Version with UnboundLocalError fix)
    """
    station_data_folder = os.path.join('HRAStationDataAnalysis', 'StationData', 'nurFiles', date)
    cuts_data_folder = os.path.join('HRAStationDataAnalysis', 'StationData', 'cuts', date)
    ...
    for station in unique_stations:
        ...
        # === Step 2: Load cuts mask for the station ===
        cuts_mask_for_time_valid = None
        # No need to keep cuts_data_dict reference long term
        if cuts:
            cuts_file = os.path.join(cuts_data_folder, f'{date}_Station{station}_Cuts.npy')
            if os.path.exists(cuts_file):
                try:
                    # Load and process within this block to limit scope
                    ...
                    # No need to del cuts_data_dict_local explicitly, it's local to the try block
        ...
        # --- File Iteration Loop ---
        for fpath in time_files:
            if not requests: break # Stop if all requests met

            # ***** FIX: Initialize variables to None before the try block *****
            times = None
            param_array = None
            indices_in_file_all_cuts = None
            indices_in_file_time_valid = None
            local_time_mask = None
            local_cuts_mask = None # Also init this one
            ...
            try:
                ...
                try:
                    ...
                    if times.size == 0: continue # Skip empty file
                except Exception as e:
                ...
                indices_in_file_time_valid = np.where(local_time_mask)[0]
                num_time_valid_in_file = len(indices_in_file_time_valid)
                del local_time_mask; local_time_mask = None # Delete early

                if num_time_valid_in_file == 0:
                    continue # Skip if no time-valid events

                # Apply external cuts
                indices_in_file_all_cuts = indices_in_file_time_valid # Default
                if cuts_mask_for_time_valid is not None:
                    start_idx = cumulative_time_valid_count
                    end_idx = cumulative_time_valid_count + num_time_valid_in_file
                    ...
                    # Delete intermediates if created
                    if local_cuts_mask is not None: del local_cuts_mask; local_cuts_mask = None
                    if indices_in_file_all_cuts is not indices_in_file_time_valid:
                        del indices_in_file_time_valid; indices_in_file_time_valid = None # indices_in_file_time_valid no longer needed
                ...
                if relevant_requests:
                    # Load parameter file only if needed for this file
                    ...
                    if param_file_path == fpath and parameter_name != 'Times':
                         ...
                         for final_idx in relevant_requests: # Iterate only over keys needed here
                            targets = requests[final_idx]
                            for event_ref, target_st_key, target_pos in targets:
                                event_ref["stations"][target_st_key][parameter_name][target_pos] = np.nan
                            del requests[final_idx] # Remove handled (failed) request
                         # No point continuing param loading for this file, but need to update counters
                    else:
                        # Proceed to load params
                        param_loaded = False
                        try:
                        ...
                        # Fulfill requests using loaded data
                ...
                # Clean up memory using variables initialized before the try block
                ...
                # Clean up others that might persist if 'continue' was hit early
        ...
        if requests:
            print(f"Warning: Station {station}: {len(requests)} requests remaining after processing files.")
            for final_idx, targets in requests.items():
    ...
    ic("Finished adding parameter (file-by-file method v2).")
