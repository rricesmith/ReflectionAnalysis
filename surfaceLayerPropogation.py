...
det = detector.Detector(source='json',json_filename='configurations/gen2_4LPDA_PA_15m_RNOG_300k_200mdipole.json',assume_inf=False)
...
#				Zen, Azi = np.asarray(getAngles.findZenAziFromSpiceDataLPDAs(depth)[3])*units.deg
				station.set_parameter(stnp.zenith,0*units.deg)
				station.set_parameter(stnp.azimuth,0*units.deg)
...
#					channel.set_trace_start_time(channel.get_trace_start_time()-time_offsets[channel.get_id()])
				...
				x1 = [-10. * units.m, -4.0 * units.m]  # SPICE at 800m
				x2 = [0.* units.m, -3.0* units.m]  # ARA antanna
				solution = r.find_solutions(x1, x2)
				print(solution)
				C_0 = solution[0]['C0']
				max_detector_freq = 0.5*units.GHz
				"""
#				print(f'station e fields {station.get_electric_fields()}')
				freq = station.get_electric_fields()[0].get_frequencies()
				r = 1
				att = 1 * (1/r) * math.exp(-r/400)
				print(f'att is {att}')
#				att = r.get_attenuation_along_path(x1, x2, C_0, freq, max_detector_freq)
				...
				print(f'max etheta {np.max(etheta)} max ephi {np.max(ephi)}')
				e_times = station.get_electric_fields()[0].get_times()
				h_etheta = hilbert(etheta)
				h_ephi = hilbert(ephi)
				print(f'h etheta {h_etheta} h ephi {h_ephi}')
				h2 = np.sqrt(h_etheta**2 + h_ephi**2)
...
if __name__== "__main__":
    parser = argparse.ArgumentParser(description='Simulate surface propogation of signals')
    parser.add_argument('nurFile', type=str, help='imput nur file to run on')

    args = parser.parse_args()
    nurFile =  args.nurFile

    depths, polData, ampsT, ampsP, EtimeT, EtimeP = findElectricFieldProperties(nurFile, [0,1,2,3])   
    template = NuRadioRecoio.NuRadioRecoio(nurFile)
