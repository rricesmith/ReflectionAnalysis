...
#        channelBandPassFilter.run(evt, station, det, passband=[1*units.MHz, 500 * units.MHz],
    ...
    def get_400s_response(self, frequencies):
        ...
        interp = interp1d(f/units.GHz, d, kind='nearest', bounds_error=False, fill_value=-60)
        return  10**(interp(frequencies/units.GHz)/20)
    ...
    def _detector_simulation_trigger(self, evt, station, det):
        ...
        # elif amp == '400':
        Vrms = 2.5*units.mV   #Ask Manuel/Steve. Steve said 2.7, Manuel said 2.5
        for channel in station.iter_channels():
            ...
            # plt.plot(frequencies, np.abs(trace_fft), label='postamp')
            # plt.legend()
            # plt.yscale('log')
            # plt.savefig(f'test400s.png')
            # plt.clf()
